...
class ImgTrainer():
    ...
    def _createProcessor(self):
        args = self.config["processor"]["args"] if self.config["processor"]["args"] is not None else {}
        # self.processor = getClass(self.config["processor"]["type"]).from_pretrained(**args)
        self.processor = getClass(self.config["processor"]["type"])(**args)
